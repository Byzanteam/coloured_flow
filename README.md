...
defmodule TrafficLight do
  ...
  def setup_flow do
    ...
    |> Ecto.Changeset.cast_embed(:data,
      with: fn schema, params ->
        Ecto.Changeset.cast(schema, params, [:definition])
      end
    )
  ...
  def setup_enactment(flow, initial_markings) do
    %Schemas.Enactment{}
    |> Ecto.Changeset.cast(%{data: %{initial_markings: initial_markings}}, [])
    |> Ecto.Changeset.put_assoc(:flow, flow)
    |> Ecto.Changeset.cast_embed(:data,
      with: fn schema, params ->
        Ecto.Changeset.cast(schema, params, [:initial_markings])
      end
    )
    |> TrafficLight.Repo.insert!()
  end
...
defmodule TrafficLight.WorkitemPubSub do
  ...
  defp get_light(%Schemas.Workitem{} = workitem) do
    dir =
      case workitem.data.binding_element.transition do
        "turn_red_" <> dir -> dir
